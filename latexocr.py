...
from PyQt6.QtWidgets import QMainWindow, QApplication, QMessageBox, QVBoxLayout, QWidget,\
    QPushButton, QTextEdit, QFormLayout, QHBoxLayout, QDoubleSpinBox,QGroupBox,QRadioButton
...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        self.radioButtonGroup = QGroupBox('Output Format', self)

        self.latexRadioButton = QRadioButton('LaTeX', self)
        self.latexRadioButton.setChecked(True)  # Set LaTeX as the default selection

        self.mathmlRadioButton = QRadioButton('MathML', self)

        radioLayout = QVBoxLayout()
        radioLayout.addWidget(self.latexRadioButton)
        radioLayout.addWidget(self.mathmlRadioButton)
        ...
        lay.addWidget(self.radioButtonGroup, stretch=1)
        ...
        self.latexRadioButton.clicked.connect(self.handleRadioButtonClicked)
        self.mathmlRadioButton.clicked.connect(self.handleRadioButtonClicked)
    ...
    def handleRadioButtonClicked(self):
        if self.latexRadioButton.isChecked():
            if self.prediction == None:
                pass
            else:
                self.displayPrediction(self.prediction)
                pp.copy(self.prediction)
        elif self.mathmlRadioButton.isChecked():
            if self.prediction == None:
                pass
            else:
                self.displayPrediction(self.prediction)
                pp.copy(parser_latex(self.prediction))
    ...
    def returnSnip(self, img=None):
        ...
        self.thread.finished.connect(self.handleRadioButtonClicked)
        self.thread.finished.connect(self.thread.deleteLater)
        self.thread.start()

    def returnPrediction(self, result):
        self.toggleProcessing(False)
        success, self.prediction = result["success"], result["prediction"]
        if success:
            self.displayPrediction(self.prediction)
            self.retryButton.setEnabled(True)
        else:
